...
// Helper: map display name to API personal color type
...
// Helper: make a short brand + product name
const parseBrandAndName = (name: string): { brand: string; title: string } => {
  const parts = name.split(' - ');
  if (parts.length >= 2) {
    return { brand: parts[0], title: parts.slice(1).join(' - ') };
  }
  // Hijab seed data has no brand; keep UI consistent
  return { brand: 'Hijab', title: name };
};
...
export const HijabCarousel: React.FC<HijabCarouselProps> = ({ personalColor }) => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const { data } = await ProductAPI.getProducts({ category: 'hijab', personalColor: pc });
        if (mounted) {
          // Keep only first 3 to match current design grid
  ...
  return (
    <div className="bg-white rounded-2xl p-4 md:p-6">
      <h3 className="text-xl font-bold text-gray-900 mb-4">Recommended Hijabs</h3>
      <div className="grid grid-cols-3 gap-3 md:gap-4">
        ...
        {!loading && items.map((product) => {
          ...
          return (
            <div
              key={product.id}
              onClick={() => handleHijabClick(product.shopeeLink)}
        ...
        {!loading && items.length === 0 && (
          <div className="col-span-3 text-center text-sm text-gray-500 py-4">등록된 히잡 상품이 없습니다.</div>
        )}
      </div>
    </div>
  );
